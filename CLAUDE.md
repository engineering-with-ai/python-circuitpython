...
## CircuitPython IoT Guidelines 📟

### Client-Driver Pattern
- **Use client-driver patterns** established in the `temperature/` example. Use it as a reference when creating new sensors.
- **Driver:** Raw hardware reads via Adafruit libraries (e.g., `adafruit_dht`)
- **Client:** Wraps driver with unit conversion and high-level API

### Async MQTT Publishing
- **Use `aiomqtt`** for async MQTT message publishing
- **Use `async with` context manager** for automatic MQTT client cleanup
- **Publish sensor readings to typed MQTT topics**

### Application Loop
- **Use `MODE` env var** to control loop behavior: `"development"` runs one iteration and returns, `"beta"` runs infinite loop
- **Use `SYSTEM_RATE` constant** to control loop frequency in seconds

### Remote Device Testing
- **Use `pytest-xdist` over SSH** for running tests on physical devices (e.g., Raspberry Pi)
- **Use `--rsyncdir`** to sync source and test files to device before execution
- **Configure `PI_HOST` env var** for target device address
